# Name: Haruka Banin
# OSU Email: baninh@oregonstate.edu
# Course: CS261 - Data Structures
# Assignment: 6
# Due Date: 03/13/2025
# Description: Hash table that uses quadratic open addressing for collision resolution
...
class HashMap:
    ...
    def put(self, key: str, value: object) -> None:
        ...
        """
        # if load factor is greater than equal to 1, resize double the capacity
        if self.table_load() >= 0.5:
            self.resize_table(self._capacity * 2)

        hash = self._hash_function(key)
        idx = hash % self._capacity
        ...
        if entry is None:
            ...
            # key is same, but it's tombstone - update the key and the value
            if entry.is_tombstone:
                entry.value = value
                entry.is_tombstone = False
                self._size += 1
            # no tombstone - just update the value
            else:
                entry.value = value
            return

        # entry exists but key doesn't match - hash collision!
        j = 1
        initial_idx = idx
        while True:
            idx = (initial_idx + j**2) % self._capacity
            entry = self._buckets[idx]
            if entry is None:
                # no value at index - insert
                self._buckets[idx] = HashEntry(key, value)
    ...
    def get(self, key: str) -> object:
        """
        Returns the value associated with the given key - if the key doesn't exist, return None
        """
        hash = self._hash_function(key)
        idx = hash % self._capacity
        entry = self._buckets[idx]
    ...
    def remove(self, key: str) -> None:
        ...
        if entry is None:
            ...
            if entry.is_tombstone:
                ...
                # find the value - update tombstone and the size
        ...
        while True:
            ...
            entry = self._buckets[idx]
            if entry is None:
                # finished probing without finding value - nothing to do
                return
            elif entry.key == key:
                if entry.is_tombstone:
                    # already removed - nothing to do
